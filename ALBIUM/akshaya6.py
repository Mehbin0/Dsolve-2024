...
c.execute("CREATE TABLE IF NOT EXISTS T_leave(TEACHER_ID INTEGER NOT NULL , TEACHER_NAME VARCHAR(50) NOT NULL, DEPARTMENT VARCHAR(50),REASON VARCHAR(50) , PHONE_NO VARCHAR(10));")
...
def leave_tec():
    ...
    head=Label(ded1,text='Apply for leave',bg='#753700',fg='#FFB012',font=("Times", 24, "bold italic"))
    ...
    def sub_2():
        ...
        adss=add.get()
        phno=phn.get()
        que1="INSERT INTO T_leave(TEACHER_ID,TEACHER_NAME,DEPARTMENT,REASON,PHONE_NO) VALUES(%s,%s,%s,%s,%s);"
        c1.execute(que1,(d_id,d_nam,de_p,adss,phno))
        con.commit()

    Button(ded1,width=38,pady=5,text='SUBMIT',bg='#fce8f3',fg='#900C3F',border=3,command=sub_2).place(x=324,y=370)
    ded1.mainloop()
    ...
    def deta1():
        ...
        colum=('TEACHER_ID','TEACHER_NAME','DEPARTMENT','REASON','PHONE_NO')
        ...
        treeview.column('REASON',width=100,anchor=CENTER)
        ...
        c1=con.cursor()
        c1.execute('SELECT * FROM T_leave')
        k=c1.fetchall()
        i=1
        for m in k :
            treeview.insert('',i,values=(m[0],m[1],m[2],m[3],m[4]))
            i+=1
        dod2.mainloop()

    Button(ded1,width=38,pady=5,text='See Leave Applications',bg='#fce8f3',fg='#900C3F',border=3,command=deta1).place(x=324,y=350)
    ded1.mainloop()


def sub_tec():
    ded2=Toplevel(root)
    ded2.title('SUBSTITUTION AVAILABILITY')
    ded2.geometry('960x435')
    ded2.iconbitmap('Med.ico')
    ded2.config(bg='#ba95a9')
    ded2.resizable(False,False)
    img=PhotoImage(file="HOS1.png")
    Label(ded2,image=img,bg='White').place(x=1,y=1)

    head=Label(ded2,text='Available slots',bg='#753700',fg='#FFB012',font=("Times", 24, "bold italic"))
    head.place(x=350,y=20)
    ...
    def deta2():
        ...
        dod3=Toplevel(root)
        dod3.title('Table')
        dod3.geometry("920x435")
        dod3.iconbitmap('Med.ico')
        dod3.configure(bg='#fce8f3')
        dod3.resizable(False,False)

        img=PhotoImage(file='HOS1.png')
        Label(dod2,image=img,bg='white').place(x=0,y=0)

        colum=('TEACHER_ID','TEACHER_NAME','DEPARTMENT','REASON','PHONE_NO')
        treeview=ttk.Treeview(dod3,height=10,show= 'headings', column= colum )
    ...
    Button(ded2,width=38,pady=5,text='See available slots',bg='#fce8f3',fg='#900C3F',border=3,command=deta1).place(x=324,y=350)
    ded2.mainloop()
...
def signin():
    ...
    if username=='host8435' and password=='Hos_t':
        ...
        head=Label(scr,text='welcome to TIOM',bg='#753700',fg='#FFB012',font=('Rockwell',33))
        head.place(x=35,y=20)
        
       
        Button(scr,width=39,pady=5,text='LEAVE APPLICATION',bg='#FFC576',fg='#900C3F',border=3,command=leave_tec).place(x=325,y=175)
        Button(scr,width=39,pady=5,text='SUBSTITION AVAILABILITY',bg='#FFC576',fg='#900C3F',border=3,command=sub_tec).place(x=325,y=210)
        scr.mainloop()
